...
export const updateIndex = ({ indexed, pks }, row, action) => { /// por que estaba asi si row es rows? export const updateIndex = ({ indexed, pks }, row, action) => {
  const key = pks.reduce((p, pk) => p + '-' + pk, '').slice(1);//const key = row[pks.reduce((p, pk) => pk, '')];// antes  
  
  switch (action?.toLowerCase()) {
    case 'insert':
      indexed[key] = row;// antes  indexed[key] = row
      break;
    case 'update':
      indexed[key] = { ...indexed[key], ...row };// antes  indexed[key] = { ...indexed[key], ...row }
      break;
    case 'delete':
      delete indexed[key];
      break;
