import { useDispatch, useSelector } from "react-redux";
import { useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { 
  Button, 
  Card, 
  CardContent, 
  SvgIcon, 
  Typography,
  Box,
  Container,
  Paper,
  IconButton,
  useTheme,
  alpha
} from '@mui/material';
import { ArrowBack } from '@mui/icons-material';
import './SignIn.css';
import { signIn, me } from "../../store/reducers/authSlice";
import { AppDispatch, RootState } from "../../store";
import VentumLogo from "../../components/VentumLogo";

const SignIn = () => {
    const dispatch = useDispatch<AppDispatch>();
    const navigate = useNavigate();
    const theme = useTheme();
    const { data } = useSelector((state: RootState) => state.auth);

    useEffect(() => {
        // Check if user is already authenticated
        console.log('SignIn: Checking authentication...');
        dispatch(me());
    }, [dispatch]);

    useEffect(() => {
        // If user is authenticated, redirect to dashboard
        console.log('SignIn: Auth data changed:', data);
        console.log('SignIn: User:', data.user);
        console.log('SignIn: Email:', data.email);
        
        const isAuthenticated = data.user || data.email;
        if (isAuthenticated) {
            console.log('SignIn: User is authenticated, redirecting to dashboard...');
            navigate('/dashboard');
        }
    }, [data.user, data.email, navigate]);

    const GoogleIcon = () => {
        return (<SvgIcon>
            <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M15.68 8.18182C15.68 7.61455 15.6291 7.06909 15.5345 6.54545H8V9.64364H12.3055C12.1164 10.64 11.5491 11.4836 10.6982 12.0509V14.0655H13.2945C14.8073 12.6691 15.68 10.6182 15.68 8.18182Z"
                    fill="#4285F4"
                />
                <path
                    d="M8 16C10.16 16 11.9709 15.2873 13.2945 14.0655L10.6982 12.0509C9.98545 12.5309 9.07636 12.8218 8 12.8218C5.92 12.8218 4.15273 11.4182 3.52 9.52727H0.858182V11.5927C2.17455 14.2036 4.87273 16 8 16Z"
                    fill="#34A853"
                />
                <path
                    d="M3.52 9.52C3.36 9.04 3.26545 8.53091 3.26545 8C3.26545 7.46909 3.36 6.96 3.52 6.48V4.41455H0.858182C0.312727 5.49091 0 6.70545 0 8C0 9.29455 0.312727 10.5091 0.858182 11.5855L2.93091 9.97091L3.52 9.52Z"
                    fill="#FBBC05"
                />
                <path
                    d="M8 3.18545C9.17818 3.18545 10.2255 3.59273 11.0618 4.37818L13.3527 2.08727C11.9636 0.792727 10.16 0 8 0C4.87273 0 2.17455 1.79636 0.858182 4.41455L3.52 6.48C4.15273 4.58909 5.92 3.18545 8 3.18545Z"
                    fill="#EA4335"
                />
            </svg>
        </SvgIcon>
        );
    };

    return (
        <Box sx={{ 
            minHeight: '100vh',
            background: `linear-gradient(135deg, ${alpha(theme.palette.grey[500], 1)} 0%, ${alpha(theme.palette.grey[300], 1)} 100%)`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            position: 'relative'
        }}>
            {/* Back to Landing Button */}
            <IconButton
                onClick={() => navigate('/')}
                sx={{
                    position: 'absolute',
                    top: 20,
                    left: 20,
                    backgroundColor: 'grey.300',
                    boxShadow: 2,
                    '&:hover': {
                        backgroundColor: 'grey.500',
                    }
                }}
            >
                <ArrowBack />
            </IconButton>

            <Container maxWidth="sm">
                <Paper
                    elevation={6}
                    sx={{
                        borderRadius: 3,
                        overflow: 'hidden',
                        background: 'linear-gradient(145deg, #ffffffff 0%, #f8f9fa 100%)'
                    }}
                >
                    {/* Header Section */}
                    <Box sx={{ 
                        p: 4, 
                        textAlign: 'center',
                        borderBottom: 1,
                        borderColor: 'divider'
                    }}>
                        <Box sx={{ mb: 3, cursor: 'pointer' }} onClick={() => navigate('/')}>
                            <VentumLogo size="large" />
                        </Box>
                        <Typography variant="h4" component="h1" sx={{ 
                            fontWeight: 'bold', 
                            mb: 1,
                            color: 'grey.600'
                        }}>
                            Welcome Back
                        </Typography>
                        <Typography variant="body1" color="grey.400">
                            Sign in to your Ventum Framework dashboard
                        </Typography>
                    </Box>

                    {/* Sign In Section */}
                    <CardContent sx={{ p: 4 }}>
                        <Button
                            variant="outlined"
                            size="large"
                            fullWidth
                            startIcon={<GoogleIcon />}
                            onClick={() => dispatch(signIn())}
                            sx={{
                                py: 2,
                                fontSize: '16px',
                                fontWeight: 600,
                                borderColor: 'divider',
                                color: 'grey.500',
                                backgroundColor: 'white',
                                boxShadow: 1,
                                '&:hover': {
                                    backgroundColor: 'grey.500',
                                    boxShadow: 2,
                                }
                            }}
                        >
                            Sign in with Google
                        </Button>

                        <Box sx={{ mt: 4, textAlign: 'center' }}>
                            <Typography variant="body2" color="text.secondary">
                                By signing in, you agree to our{' '}
                                <Typography 
                                    component="span" 
                                    variant="body2" 
                                    color="primary" 
                                    sx={{ cursor: 'pointer', textDecoration: 'underline' }}
                                >
                                    Terms of Service
                                </Typography>
                                {' '}and{' '}
                                <Typography 
                                    component="span" 
                                    variant="body2" 
                                    color="primary" 
                                    sx={{ cursor: 'pointer', textDecoration: 'underline' }}
                                >
                                    Privacy Policy
                                </Typography>
                            </Typography>
                        </Box>
                    </CardContent>

                    {/* Footer */}
                    <Box sx={{ 
                        p: 3, 
                        backgroundColor: alpha(theme.palette.primary.main, 0.04),
                        textAlign: 'center'
                    }}>
                        <Typography variant="body2" color="text.secondary">
                            New to Ventum Framework?{' '}
                            <Typography 
                                component="span" 
                                variant="body2" 
                                color="primary" 
                                onClick={() => navigate('/')}
                                sx={{ cursor: 'pointer', fontWeight: 600 }}
                            >
                                Learn more
                            </Typography>
                        </Typography>
                    </Box>
                </Paper>
            </Container>
        </Box>
    );
};

export default SignIn;





